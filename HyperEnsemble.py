...
from hypernets.tabular import ensemble
import pandas as pd
import numpy as np
import hashlib
...
class HyperEnsembel:
    def __init__(self,estimators,X_train,y_train,scorer, task,X_eval=None,y_eval=None,ensemble_size=20,random_seed=623):
...
ensemble_ = HyperEnsembel(estimators=estimators,X_train=X,y_train=y,scorer=get_scorer(metric_to_scoring(reward_metric)),task='binary',random_seed=0,ensemble_size=9)
